...
### 2026-06-28 自作レビューを高機能化（全幅カルーセル/全文モーダル/評価サマリー）
- 全幅・**1カードずつ自動送り**（5枚＝幅1/5で中央に1枚）／本文**3行で「…」省略**＋デモを長文化
- **カードクリックで全文モーダル**（フェードイン＋「この商品を見る」ボタン）。`<body>`直下へ移動して暗幕を全画面化、背景クリック/ESC/×で閉じる
- **見出し下に評価サマリー**（加重平均★・件数、半端は星を幅%で部分塗り）
- 名前・商品名を**1行で省略**（meta-line ellipsis）。設定追加：表示枚数/行数/星色/カード色/速さ/各表示ON/OFF
- ★重要な学び：Dawnの `{% stylesheet %}` は一部ルール（modal-overlay/box・line-clamp）を**配信から落とす**ことがある→**重要CSSはインライン`<style>`で確実化**。`if`条件内で `|` フィルターは使用不可（変数化する）
- デモを長文版CSV（`komorebi-reviews-long.csv`）で入れ直し（旧14件Delete→再Import）。**再importはメタフィールド同期に数分ラグ**（アプリ版=API即時／自作=メタフィールド読みは遅延）
- **次回TODO：レビューの★（星）の大きさを大きくする**
- コミット：このセッションでまとめてpush
